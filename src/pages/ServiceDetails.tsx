...
export default function ProductDetails() {
  ...
  if (isLoading) {
    ...
    return (
      ...
      >
        <div className="text-xl text-[#ffd453]">جاري التحميل...</div>
      </div>
    );
  }
  ...
  if (error || !service) {
    ...
    return (
      <div
        ...
        style={backgroundStyles}
      >
        <div className="text-xl text-[#ffd453]">{error || 'المنتج غير موجود'}</div>
        <button
          onClick={() => navigate('/')}
          className="bg-[#ffd453] text-[#1c594e] px-6 py-2 rounded-md font-bold hover:brightness-110 transition-all"
        >
          العودة للرئيسية
        </button>
      </div>
      ...
      <div className="min-h-screen flex flex-col pt-24" style={backgroundStyles}>
            ...
            <div className="rounded-md shadow-lg overflow-hidden glass">
              <div className="md:flex">
                ...
                <div className="md:w-1/2 p-8">
                  <h1 className="text-3xl font-bold mb-4 text-[#ffd453]">{service.title}</h1>
                  <p className="text-white mb-6 text-lg leading-relaxed">
                    {(currentLanguage === 'en' && service.description_en && service.description_en.trim()) 
                      ? service.description_en 
                      : service.description}
                  </p>
                  <div className="border-t border-[#ffd453]/20 pt-6 mb-6">
                    <div className="text-2xl font-bold text-[#ffd453] mb-6">
                      {service.price} {currentLanguage === 'ar' ? 'ج.م' : 'EGP'}
                    </div>
                    <div className="flex flex-col gap-4">
                      <button
                        onClick={handleContact}
                        className="w-full bg-[#ffd453] text-[#1c594e] font-bold py-4 px-6 rounded-md hover:brightness-110 transition-all flex items-center justify-center gap-2 shadow-lg"
                      >
                      ...
                      <a
                        ...
                        className="w-full bg-[#26bd7e] text-[#1c594e] py-3 px-6 rounded-md font-bold hover:brightness-110 transition-all flex items-center justify-center gap-2"
                      ...
                      {/* نص توضيحي للمشاركة */}
                      <p className="text-xs text-white/60 mt-2 text-center w-full">يمكنك مشاركة رابط المنتج مع أصدقائك على واتساب وسيظهر لهم صورة المنتج تلقائيًا</p>
                    </div>
                  </div>
                </div>
              </div>
        ...
        {suggested.length > 0 && (
          <div className="container mx-auto px-4 max-w-4xl lg:max-w-5xl mb-8">
            <h2 className="text-xl font-bold text-[#ffd453] mb-4">{t('products.alsoAvailable')}</h2>
            <div
                ...
                return (
                  <div
                    ...
                    className="
                      ...
                      bg-white/5 border border-[#ffd453]/10 rounded-md shadow p-2 flex-shrink-0 cursor-pointer hover:scale-105 transition
                    "
